...
const apiURLDetail = 'http://127.0.0.1:8000/api/indexpodetail';
let purchaseOrderDetail = [];
let filteredDataDetail = [];

async function fetchPurchaseOrdersDetail() {
    try {
        console.log("Fetching data...");
        ...
        purchaseOrderDetail = data.data.map(po => ({
            noPO: po.po_no.toString(),
            parNo: po.po_po_no,
        }));

        // Set filteredData to match purchaseOrderDetail initially
        filteredDataDetail = purchaseOrderDetail;

        // Call functions to display data and setup pagination
        displayPODetails(filteredDataDetail);
        updatePagination();
    } catch (error) {
        console.error('Error fetching purchase orders:', error);
    }
