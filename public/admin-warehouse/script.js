const baseURL = 'https://api.edutrashgo.com/api';
  let deliveryNote = [];
  let filteredDnData = [];
  let currentPage = 1;
  ...
  document.addEventListener('DOMContentLoaded', () => {
      ...
      // Ambil nomor DN dari URL
      const urlParams = new URLSearchParams(window.location.search);
      const noDN = urlParams.get('noDN');

      // Masukkan nomor DN ke dalam elemen dengan id "dnNumber"
      const dnNumberElement = document.getElementById('dnNumber');
      if (noDN && dnNumberElement) {
          dnNumberElement.textContent = noDN; // Set the DN number in the div
      } else {
          console.error('No DN number found in URL or element with id "dnNumber" not found.');
      }

      // Ambil data DN dari localStorage dan tampilkan
      const dnDetail = JSON.parse(localStorage.getItem('dnDetail'));
      if (dnDetail) {
          displayDNDetails(dnDetail); // Function to render details to the page
      } else {
          console.error('No DN details found in localStorage!');
      }
  });
  ...
  // Fungsi untuk menampilkan detail DN
